...
class MarkataTUI(App):
    CSS_PATH = "style.css"
    BINDINGS = [("q", "quit", "Quit"), (":", "focus_command", "Command Mode")]

    def __init__(self):
        super().__init__()
        self.m = Markata()
        # self.m.run()
        self.command_input = Input(placeholder=":command", id="command")
        self.output = Static("Welcome to Markata TUI", id="output")

    def compose(self) -> ComposeResult:
        # yield Header(show_clock=True)
        yield Static(
            f"[b]Site:[/] {self.m.config.site_name} \n[b]Author:[/] {self.m.config.author_name} <{self.m.config.author_email}>",
            id="meta",
        )
    ...
    def on_input_submitted(self, message: Input.Submitted) -> None:
        ...
        if command == ":plugins" or command == ":plug":
            ...
            titles = [post.title for post in self.m.posts]
            self.output.update(Panel("\n".join(titles), title="Posts"))

        elif command == ":feeds" or command == ":feed":
            feeds = [f"{k}: {len(v)} posts" for k, v in self.m.feeds.items()]
